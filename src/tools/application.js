...
export function registerApplicationTools(server) {
  server.registerTool(
    'application_create',
    {
      description: 'Create an application. Returns applicationId and the one-time default password.',
      inputSchema: {
        code: z.string().describe('Application code, pattern [A-Za-z0-9][A-Za-z0-9-_]*. Lowercased.'),
        unitId: z.string().describe('Associated unit ID.'),
        hostUri: z.string().describe('Application queue URI (e.g. amqp://host/vhost).'),
    ...
    async ({ code, unitId, hostUri, name, info, ttl, length }) => {
      ...
      try {
        const data = await coremgr.request('POST', '/api/v1/application', { body: { data: dataFields } });
        return ok(data);
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    'application_count',
    {
      description: 'Get the count of applications in a unit.',
      inputSchema: {
    ...
    async ({ unit, contains }) => {
      try {
        const data = await coremgr.request('GET', '/api/v1/application/count', { params: { unit, contains } });
        return ok(data);
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    'application_list',
    {
      description: 'List applications in a unit.',
      inputSchema: {
        unit: z.string().describe('Unit ID (required for normal users).'),
    ...
    async ({ unit, contains, offset, limit, sort }) => {
      try {
        const data = await coremgr.request('GET', '/api/v1/application/list', {
  ...
  server.registerTool(
    'application_get',
    {
      ...
      inputSchema: {
        applicationId: z.string().describe('The application ID to retrieve.'),
      },
    },
    async ({ applicationId }) => {
      try {
        const data = await coremgr.request('GET', `/api/v1/application/${applicationId}`);
        return ok(data);
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    'application_update',
    {
      description: 'Update an application. password is required when changing hostUri.',
      inputSchema: {
    ...
    async ({ applicationId, hostUri, name, info, ttl, length, password }) => {
      const dataFields = {};
      ...
      try {
        await coremgr.request('PATCH', `/api/v1/application/${applicationId}`, { body: { data: dataFields } });
  ...
  server.registerTool(
    ...
    async ({ applicationId }) => {
      try {
        await coremgr.request('DELETE', `/api/v1/application/${applicationId}`);
  ...
  server.registerTool(
    'application_stats',
    {
      description: 'Get queue statistics for an application (uldata, dldataResp, dldataResult queues).',
      inputSchema: {
    ...
    async ({ applicationId }) => {
      try {
        const data = await coremgr.request('GET', `/api/v1/application/${applicationId}/stats`);
        return ok(data);
      } catch (e) {
        return err(e);
      }
  ...
  server.registerTool(
    ...
    {
      description: 'Send downlink data to a device via an application.',
      inputSchema: {
        applicationId: z.string().describe('The application ID.'),
    ...
    async ({ applicationId, deviceId, payload }) => {
      try {
        await coremgr.request('POST', `/api/v1/application/${applicationId}/dldata`, {
